...
/* Q1
todo: (How many languages?) */
let arr = new Set();
let emptyArr = [];
for (let i = 0; i < countries.length; i++) {

    let a = countries[i]["languages"];
    emptyArr.push(a)
};
let text = emptyArr.join(",");
let allArr = text.split(",")
let countSet = new Set(allArr)
let howMany = countSet.size
console.log(`There are "${howMany}" languages.`)
...
for (let i = 0; i < countries.length; i++) {
    let a =countries[i]["languages"];
 let filter = [...countSet].filter(e => a.includes(e));
 console.log(filter)
